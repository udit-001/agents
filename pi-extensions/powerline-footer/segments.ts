...
const usageStatusSegment: StatusLineSegment = {
  ...
  render(ctx) {
    ...
    if (typeof usage.weeklyRemainingPercent === "number" && typeof usage.weeklyResetAfterSeconds === "number") {
      ...
      const weeklyReset = formatDurationSeconds(usage.weeklyResetAfterSeconds);
      label += ` wk:${weeklyRemaining}%/${weeklyReset}`;
    }

    const weeklyRemainingForThreshold = typeof usage.weeklyRemainingPercent === "number"
      ? usage.weeklyRemainingPercent
      : null;

    const minRemaining = weeklyRemainingForThreshold === null
      ? remaining
      : Math.min(remaining, weeklyRemainingForThreshold);

    let semantic: SemanticColor = "tokens";
    if (usage.limitReached || minRemaining <= 5) {
      semantic = "contextError";
    } else if (minRemaining <= 20) {
      semantic = "contextWarn";
    }

    return { content: color(ctx, semantic, label), visible: true };
  },
};
